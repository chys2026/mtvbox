...
public class EXOmPlayer extends ExoMediaPlayer {
    ...
    public EXOmPlayer(Context context) {
        super(context);
        configureLoadErrorHandling();
    }

    private void configureLoadErrorHandling() {
        if (mMediaPlayer != null) {
            mMediaPlayer.setWakeMode(C.WAKE_MODE_NETWORK);
        }
    }

    public void setPlaybackSpeed(float speed) {
        if (mMediaPlayer != null) {
            mMediaPlayer.setPlaybackParameters(new PlaybackParameters(speed, 1.0f));
        }
    }

    public float getPlaybackSpeed() {
        if (mMediaPlayer != null && mMediaPlayer.getPlaybackParameters() != null) {
