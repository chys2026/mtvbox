...
abstract class BaseVbFragment<T : ViewBinding> : Fragment(), CustomAdapt {
    ...
    private fun initBindingViewRoot(container: ViewGroup?): View? {
        ...
        try {
            ...
            Log.e("BaseVbFragment", "initBindingViewRoot error", e)
        }
        return null
    }
    ...
    override fun getSizeInDp(): Float {
        return if (activity is CustomAdapt) activity.sizeInDp else 0f
    }

    override fun isBaseOnWidth(): Boolean {
        return if (activity is CustomAdapt) activity.isBaseOnWidth else true
    }
}
