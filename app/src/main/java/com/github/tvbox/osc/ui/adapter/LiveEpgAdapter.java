...
public class LiveEpgAdapter extends BaseQuickAdapter<Epginfo, BaseViewHolder> {
    ...
    private static final ThreadLocal<SimpleDateFormat> timeFormatTL = new ThreadLocal<SimpleDateFormat>() {
        @Override
    ...
    protected void convert(BaseViewHolder holder, Epginfo value) {
        ...
        if (value.index == selectedEpgIndex && value.index != focusedEpgIndex && (value.currentEpgDate.equals(shiyiDate) || value.currentEpgDate.equals(timeFormatTL.get().format(new Date())))) {
