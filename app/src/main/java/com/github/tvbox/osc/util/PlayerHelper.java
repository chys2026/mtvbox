...
public class PlayerHelper {
    ...
    public static final float[] PLAY_SPEEDS = {0.5f, 0.75f, 1.0f, 1.25f, 1.5f, 2.0f, 3.0f};
    public static final String[] PLAY_SPEED_LABELS = {"0.5X", "0.75X", "1.0X", "1.25X", "1.5X", "2.0X", "3.0X"};

    public static final int DEFAULT_BUFFERING_TIMEOUT = 30000;
    public static final int DEFAULT_PLAYBACK_TIMEOUT = 60000;

    public static int getSpeedIndex(float speed) {
        for (int i = 0; i < PLAY_SPEEDS.length; i++) {
            if (Math.abs(PLAY_SPEEDS[i] - speed) < 0.01f) return i;
        }
        return 2;
    }
    ...
    public static String getSpeedLabel(float speed) {
        ...
        if (idx >= 0 && idx < PLAY_SPEED_LABELS.length) return PLAY_SPEED_LABELS[idx];
        return "1.0X";
    }
    ...
    public static void updateCfg(VideoView videoView, JSONObject playerCfg) {
        ...
        try {
            ...
            Log.e(TAG, "Error parsing player config JSON", e);
        }
                ...
                tv.danmaku.ijk.media.player.IjkMediaPlayer.loadLibrariesOnce(new IjkLibLoader() {
                    ...
                    public void loadLibrary(String s) throws UnsatisfiedLinkError, SecurityException {
                        try {
                            ...
                            Log.e(TAG, "Error loading native library in updateCfg (player)", th);
                        }
                    }
                });
            } catch (Throwable th) {
                Log.e(TAG, "Error initializing IjkMediaPlayer libraries in updateCfg (player)", th);
            }
                ...
                tv.danmaku.ijk.media.player.IjkMediaPlayer.loadLibrariesOnce(new IjkLibLoader() {
                    ...
                    public void loadLibrary(String s) throws UnsatisfiedLinkError, SecurityException {
                        try {
                            ...
                            Log.e(TAG, "Error loading native library in updateCfg (play)", th);
                        }
                    }
                });
            } catch (Throwable th) {
                Log.e(TAG, "Error initializing IjkMediaPlayer libraries in updateCfg (play)", th);
            }
        } else if (playType == 2) {
            playerFactory = new PlayerFactory<EXOmPlayer>() {
                @Override
            ...
            tv.danmaku.ijk.media.player.IjkMediaPlayer.loadLibrariesOnce(new IjkLibLoader() {
                ...
                public void loadLibrary(String s) throws UnsatisfiedLinkError, SecurityException {
                    try {
                        ...
                        Log.e(TAG, "Error loading native library in init", th);
                    }
                }
            });
        } catch (Throwable th) {
            Log.e(TAG, "Error initializing IjkMediaPlayer libraries in init", th);
        }
    }
